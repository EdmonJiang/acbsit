...
$('nav ul a').each(function(item){
    if(~$(this).attr('href').indexOf(curl)){
...
angular.module('staffApp', [])
    ...
    .controller('adCtrl', ['$scope', '$http',function($scope, $http) {
            $scope.userInfo = "";
            $scope.username = "";
            $scope.searching = false;
            $scope.errMsg = {
    ...
    .controller('iptCtrl', ['$scope', '$http', '$sce',function($scope, $http, $sce) {
            $scope.userInfo = ""
            $scope.email = ""
            $scope.searching = false;
            $scope.errMsg = {
    ...
    .controller('snCtrl', ['$scope', '$http',function($scope, $http) {
            $scope.assetdata = "";
            $scope.assetheader = "";
            $scope.servicetag = "";
            $scope.searching = false;
    ...
    .controller('thinkCtrl', ['$scope', '$http',function($scope, $http) {
            $scope.assets = {};
            $scope.searching = false;
            $scope.errMsg = {
                text: "",
    ...
    .controller('awCtrl', ['$scope', '$http', 'paginateService', function($scope, $http, paginateService) {
            $scope.users = [];
            $scope.count = 0;
            $scope.curPage = 0;
            $scope.totalPage = 0;
            ...
            $scope.get_users = function(page) {
                ...
                $http.get('/cmdb/airwatch/invoice', { params: params }).then(function(response) {
                    ...
                    if (response.data.error) {
                        ...
                        $scope.showPage = paginateService.GetPager(page, total);
                    }
                    $scope.isLoading = false;
    ...
    .controller('symCtrl', ['$scope', '$http', 'paginateService', function ($scope, $http, paginateService) {
            $scope.users = [];
            $scope.searching = false;
            $scope.curPage = 0;
            $scope.showPage = [1];
            ...
            var get_data = function (data) {
                $http.get('/cmdb/symantec/altiris?' + data.key + '=' + data.value).then(function (response) {
                    ...
                    if (response.data.error) {
                        ...
                        $scope.curPage = 1;
                        $scope.showPage = paginateService.GetPager(1, totalPage);
                        $scope.errMsg.class = "text-success";
                        $scope.searching = false;
                    }
            ...
            $scope.get_page = function (page) {
                if (page < 1 || $scope.curPage === page || page > $scope.totalPage) {
                    ...
                    $scope.showPage = paginateService.GetPager(page, $scope.totalPage)
                }
            }

            $scope.export2csv = function (data) {
                var csv = paginateService.jsonTocsv(data,['PC Name', 'Primary User', 'PC Domain', 'OS Name', 'Computer Model', 'Serial Number', 'TPM Activated', 'Encryption Status', 'IP Address', 'Last Connected']);
                if(!csv){return;}
            ...
            $scope.getDetails = function (query, flag) {
                ...
                switch(flag){
                    ...
                    case 'SN':
                        ...
                        cb = paginateService.formatSNDate;
                        break;
                    default:
                        return;
                }
                ...
                $http.get(url).then(function(response) {
                    ...
                    if (response.data.error) {
                        ...
                        $('#txtFilter').val("");
                        $('#symModalLabel').text(title+ query[0]);
                        paginateService.json2table("table-detail", data, fields, cb);
                        $('#sym-modal').modal({ keyboard: true });
    ...
    .service('paginateService', function () {
        var service = this;
        service.GetPager = function (currentPage, totalPages) {
            ...
            return service.range(startPage, endPage + 1);
        }
        ...
        service.jsonTocsv = function (data, fields) {
        ...
        service.json2table = function (tableId, data, fields ,callback){
        ...
        service.formatSNDate = function (tbl_body){
